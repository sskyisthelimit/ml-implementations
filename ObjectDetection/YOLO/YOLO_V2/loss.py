...
class YoloLoss():
    ...
    def __call__(self, predictions, labels):
        ...
        with torch.no_grad():
            iou_pred_with_target = self.calculate_iou(
        ...
        obj_loss = self.obj_loss_func(pred_obj,
                                      iou_pred_with_target) * target_obj
        ...
        txty_loss = self.box_loss_func(pred_box_txty,
                                       target_box_txty).sum(dim=-1) * target_obj
        txty_loss = txty_loss.sum() / self.bs

        twth_loss = self.box_loss_func(pred_box_twth,
                                       target_box_twth).sum(dim=-1) * target_obj
        twth_loss = twth_loss.sum() / self.bs
        
        cls_loss = self.cls_loss_func(pred_cls, target_cls) * target_obj
        cls_loss = cls_loss.sum() / self.bs

        multipart_loss = self.lambda_obj * obj_loss + noobj_loss + (
            txty_loss + twth_loss) + cls_loss
        return [multipart_loss, obj_loss, noobj_loss,
                txty_loss, twth_loss, cls_loss]
    ...
    def build_target(self, label):
        target = torch.zeros(size=(
            self.grid_size, self.grid_size, self.num_boxes,
            self.num_attributes), dtype=torch.float32)
        ...
        if -1 in label[:, 0]:
            ...
            for item in label:
                ...
                ious_target_with_anchor = self.calculate_iou_target_with_anchors(
                    target_wh=item[3:5], anchor_wh=self.anchors)
    ...
    def build_batch_target(self, labels):
        ...
        return batch_target.view(self.bs, -1,
                                  self.num_boxes, self.num_attributes)
